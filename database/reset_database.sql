...
-- v2: reset zera sensor_training_data (dados de treino) + sensor_data (legado)
--     sensor_monitoring_data e PRESERVADA (dados de producao nao sao apagados pelo reset padrao)

-- --- Reset legado: sensor_data ---
BEGIN
...
-- --- Reset de treino: sensor_training_data ---
BEGIN
  EXECUTE IMMEDIATE 'DROP TABLE sensor_training_data PURGE';
EXCEPTION
  WHEN OTHERS THEN
...
CREATE INDEX idx_tr_device ON sensor_training_data(device_id);
CREATE INDEX idx_tr_ts ON sensor_training_data(ts_epoch);
CREATE INDEX idx_tr_col ON sensor_training_data(collection_id);
CREATE INDEX idx_tr_fan ON sensor_training_data(fan_state);
CREATE INDEX idx_tr_combo ON sensor_training_data(cmd_speed_label, rot_state_label, use_state_label, vib_profile_label);

-- NOTA: sensor_monitoring_data NAO e resetada aqui.
-- Para resetar dados de monitoramento: TRUNCATE TABLE sensor_monitoring_data;
